...
export const AICoachPanel: React.FC<AICoachPanelProps> = ({ daysPerWeek, unit }) => {
  ...
  return (
    <div className="max-w-5xl mx-auto py-8 px-4 space-y-8">
      {/* Top Selector Header */}
      <div className="bg-white border border-stone-200 rounded-3xl p-6 sm:p-8 space-y-6 shadow-xs mv-glow-cyan">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <div className="flex items-center space-x-2 text-stone-500 text-xs font-bold uppercase tracking-widest mb-1">
              <Sparkles className="w-3.5 h-3.5 text-stone-800" />
              <span>Gemini 3.6 Flash Intelligence</span>
            </div>
            <h1 className="text-3xl font-normal mv-gradient-text tracking-tight">AI Glute Specialist</h1>
      ...
      {/* Tab 1: AI Chat */}
      {activeTab === 'chat' && (
        <div className="bg-white border border-stone-200 rounded-3xl p-4 sm:p-6 space-y-4 flex flex-col h-[550px] shadow-xs">
          {/* Preset Prompts */}
          <div className="flex items-center space-x-2 overflow-x-auto no-scrollbar pb-3 border-b border-stone-100 shrink-0">
            {presetQuestions.map((q, idx) => (
              <button
                key={idx}
                onClick={() => handleSendChat(q)}
                className="bg-stone-50 hover:bg-stone-100 border border-stone-200/80 text-stone-700 text-xs font-medium px-3.5 py-2 rounded-full whitespace-nowrap transition-colors"
              >
                {q}
              </button>
            ))}
          ...
          <div className="flex-1 overflow-y-auto space-y-4 pr-1">
            {messages.map((msg, idx) => (
              ...
              >
                {msg.sender === 'ai' && (
                  <div className="w-8 h-8 rounded-full bg-stone-900 text-white flex items-center justify-center shrink-0 mv-glow-pink">
                ...
                <div
                  className={`max-w-xl rounded-2xl p-4 text-xs sm:text-sm leading-relaxed ${
                    msg.sender === 'user'
                      ...
                      : 'bg-stone-50 border border-stone-200/80 text-stone-800 rounded-tl-none whitespace-pre-wrap'
                  }`}
                >
                  {msg.text}
                </div>
            ...
            {isChatLoading && (
              <div className="flex items-center space-x-3 text-stone-500 text-xs">
                <Bot className="w-4 h-4 text-stone-900 animate-spin" />
      ...
      {activeTab === 'custom-generator' && (
        <div className="space-y-6">
          <div className="bg-white border border-stone-200 rounded-3xl p-6 sm:p-8 space-y-6 shadow-xs">
            <h2 className="text-2xl font-normal mv-gradient-text tracking-tight flex items-center space-x-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div className="space-y-1.5">
                ...
                >
                  <option value="Beginner">Beginner (Mastering Form & Activation)</option>
                  <option value="Intermediate">Intermediate (Consistent Lifter)</option>
                  <option value="Advanced">Advanced (High Frequency & Intensity)</option>
                </select>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  className="w-full bg-stone-50 border border-stone-200/80 rounded-2xl px-4 py-3 text-xs text-stone-900"
                >
                  <option value="Full Gym (Barbell, Cables, Dumbbells, Machines)">
                    Full Gym (Barbell, Cables, Dumbbells, Machines)
                  </option>
                  <option value="Dumbbells & Resistance Bands Only (Home Gym)">
                    Dumbbells & Resistance Bands Only (Home Gym)
                  </option>
                  <option value="Bodyweight & Mini Bands Only">
                    Bodyweight & Mini Bands Only
                  </option>
                </select>
              </div>
            </div>
            ...
            <button
              ...
              className="w-full py-4 bg-stone-900 hover:bg-stone-800 text-white font-medium text-xs tracking-wider uppercase rounded-full shadow-xs mv-glow-pink flex items-center justify-center space-x-2 transition-all"
            >
              {isGenLoading ? (
                <>
          ...
          {generatedResult && (
            <div className="bg-white border border-stone-200 rounded-3xl p-6 sm:p-8 space-y-6 shadow-xs">
              <div className="border-b border-stone-100 pb-4">
                ...
                <h3 className="text-2xl font-normal mv-gradient-text mt-0.5">{generatedResult.splitTitle}</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {generatedResult.days?.map((day: any, idx: number) => (
                  <div key={idx} className="bg-stone-50 p-5 rounded-2xl border border-stone-200/80 space-y-3">
                    <span className="text-[10px] font-bold text-stone-400 uppercase tracking-widest">Session 0{day.dayNumber}: {day.title}</span>
