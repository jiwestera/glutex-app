  ...
  return (
    <div className="max-w-6xl mx-auto py-8 px-4 space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <div className="bg-white border border-stone-200 rounded-3xl p-6 space-y-2 shadow-xs mv-glow-cyan">
          <div className="flex items-center justify-between text-xs font-bold text-stone-400 uppercase tracking-widest">
            <span>Workouts Completed</span>
            <Calendar className="w-4 h-4 text-stone-700" />
          </div>
          <p className="text-4xl font-serif mv-gradient-text tracking-tight">{logs.length}</p>
          <p className="text-xs text-stone-500">Total Saved Days in History</p>
        </div>

        <div className="bg-white border border-stone-200 rounded-3xl p-6 space-y-2 shadow-xs mv-glow-gold">
          <div className="flex items-center justify-between text-xs font-bold text-stone-400 uppercase tracking-widest">
            <span>Personal Records</span>
            <Award className="w-4 h-4 text-[#ffd23f]" />
          </div>
          <p className="text-4xl font-serif mv-gradient-text-gold tracking-tight">{prList.length}</p>
          <p className="text-xs text-stone-500">Heavy Lifts Tracked</p>
        </div>

        <div className="bg-white border border-stone-200 rounded-3xl p-6 space-y-2 shadow-xs mv-glow-pink">
          <div className="flex items-center justify-between text-xs font-bold text-stone-400 uppercase tracking-widest">
            <span>Consistency</span>
            <Flame className="w-4 h-4 text-stone-700" />
          </div>
          <p className="text-4xl font-serif mv-gradient-text tracking-tight">{logs.length > 0 ? 'Active' : '0'}</p>
      ...
      <div className="bg-white border border-stone-200 rounded-3xl p-6 sm:p-8 space-y-6 shadow-xs">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <h2 className="text-2xl font-normal mv-gradient-text-gold tracking-tight">Glute Strength Records</h2>
            <InfoBall
        ...
        {prList.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {prList.map((pr, idx) => (
              <div key={idx} className="bg-stone-50 p-5 rounded-2xl border border-stone-200/80 space-y-1 hover:mv-glow-gold transition-all">
                ...
                <div className="text-xl font-serif mv-gradient-text-gold">
                  {pr.maxWeight} {unit} <span className="text-xs text-stone-500 font-normal">x {pr.maxReps} reps</span>
                </div>
              ...
              return (
                <div key={log.id} className="bg-stone-50 p-5 rounded-2xl border border-stone-200/80 space-y-4">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <div>
                      <div className="flex items-center space-x-2">
                        <span className="text-[10px] font-bold text-stone-500 uppercase tracking-widest bg-stone-200/70 px-2 py-0.5 rounded-md">
                          ...
                          <div key={idx} className="bg-white p-3.5 rounded-xl border border-stone-200/80 space-y-2">
                            ...
                            <div className="flex flex-wrap gap-2 pt-1">
                              {e.sets.map((s, sIdx) => (
                                ...
                                >
                                  <span className="font-bold text-stone-500">Set {s.setNumber}:</span>
                                  <span>
      ...
      {selectedLogForModal && (
        <div className="fixed inset-0 z-50 bg-stone-900/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl border border-stone-200 max-w-2xl w-full max-h-[90vh] flex flex-col shadow-2xl mv-glow-cyan overflow-hidden animate-scaleIn">
            {/* Modal Header */}
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div className="bg-white p-3.5 rounded-2xl border border-stone-200/80 space-y-0.5">
                  ...
                  <span className="text-base font-semibold text-stone-900 flex items-center space-x-1">
                    ...
                    <span>{selectedLogForModal.exercises.length} Exercises</span>
                  </span>
                </div>
