...
export const MobilityHub: React.FC<MobilityHubProps> = ({ customWarmups = [] }) => {
  ...
  return (
    <div className="max-w-6xl mx-auto py-8 px-4 space-y-8">
      {/* Hero Header */}
      <div className="bg-white border border-stone-200 rounded-3xl p-6 sm:p-8 space-y-3 shadow-xs mv-glow-cyan">
        ...
        <h1 className="text-3xl sm:text-4xl font-normal mv-gradient-text tracking-tight">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {allRoutines.map((routine) => {
          return (
            <div
              key={routine.id}
              className="bg-white border border-stone-200 hover:border-stone-300 hover:mv-glow-pink rounded-3xl p-6 space-y-5 transition-all shadow-xs flex flex-col justify-between"
            >
              <div className="space-y-4">
                ...
                <div className="flex items-start justify-between gap-2">
                  <h3 className="text-xl font-normal mv-gradient-text tracking-tight">{routine.name}</h3>
                  <InfoBall
                    title={routine.name}
