...
export const AddDayModal: React.FC<AddDayModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-stone-950/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-6 overflow-y-auto">
      <div className="bg-white border border-stone-200 rounded-3xl overflow-hidden max-w-2xl w-full relative shadow-2xl mv-glow-pink text-stone-900 my-auto max-h-[92vh] flex flex-col">
        {/* Header */}
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 sm:p-8 py-5 space-y-4 bg-white">
          ...
          <div className="flex items-center justify-end space-x-3 pt-3 border-t border-stone-100">
            ...
            <button
              type="submit"
              className="px-6 py-2.5 rounded-full text-xs font-semibold uppercase tracking-wider text-white bg-stone-900 hover:bg-stone-800 transition-all shadow-md mv-glow-cyan flex items-center space-x-2"
            >
              <Plus className="w-4 h-4" />
              <span>Add Day {nextDayNumber}</span>
            </button>
          </div>
        </form>
      </div>
