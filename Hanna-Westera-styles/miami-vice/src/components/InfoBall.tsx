  ...
  return (
    <div className={`relative inline-flex items-center shrink-0 ${className}`} ref={containerRef}>
      <button
        ...
        className={`${sizeClasses[size]} rounded-full bg-stone-200 hover:bg-stone-900 text-stone-700 hover:text-white font-bold font-serif flex items-center justify-center transition-colors shadow-2xs cursor-pointer focus:outline-hidden`}
      ...
      {isOpen && (
        <div
          ...
          className={`absolute z-50 w-64 sm:w-72 max-w-[calc(100vw-32px)] p-3.5 bg-stone-900 text-stone-100 border border-stone-800 rounded-2xl shadow-xl mv-glow-cyan animate-fadeIn ${getPopoverPositionClasses()} pointer-events-auto`}
        >
