...
async function startServer() {
  ...
  // is billed to this server's own key, so unthrottled traffic (accidental
  // or abusive) directly costs money. Simple in-memory fixed-window limiter,
  ...
  setInterval(() => {
    ...
    for (const [key, timestamps] of aiRequestLog) {
      const recent = timestamps.filter((t) => now - t < AI_RATE_LIMIT_WINDOW_MS);
      if (recent.length === 0) aiRequestLog.delete(key);
      else aiRequestLog.set(key, recent);
    }
  }, 5 * 60 * 1000).unref();
  ...
  const aiRateLimit: express.RequestHandler = (req, res, next) => {
    ...
    const now = Date.now();
    const timestamps = (aiRequestLog.get(key) || []).filter(
  ...
  app.post("/api/ai/coach", aiRateLimit, async (req, res) => {
    try {
...
4. If asked about injuries, gently recommend consulting a physical therapist while giving safe movement regressions.`;
  ...
  app.post("/api/ai/generate-split", aiRateLimit, async (req, res) => {
    try {
