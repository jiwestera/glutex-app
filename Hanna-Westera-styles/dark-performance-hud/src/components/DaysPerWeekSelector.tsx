...
export const DaysPerWeekSelector: React.FC<DaysPerWeekSelectorProps> = ({
  ...
  const handleDeleteSplit = (e: React.MouseEvent, split: WorkoutSplit) => {
    e.stopPropagation();
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto py-8 px-4">
      {/* Hero Banner */}
      <div className="bg-white border border-stone-200 rounded-3xl p-6 sm:p-8 shadow-xs space-y-4 hud-corners">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {allAvailableSplits.map((split) => {
            ...
            return (
              ...
              >
                {isSelected && (
                  <div className="absolute -top-3 left-1/2 transform -translate-x-1/2 bg-stone-950 border border-cyan-400/60 text-cyan-300 text-[9px] font-bold uppercase tracking-widest px-3 py-0.5 rounded-full shadow-2xs glow-border-cyan hud-live-dot">
                    Active
                  </div>
                )}
                ...
                <div>
                  <div className="flex items-baseline justify-between mb-2">
                    <span className={`text-3xl font-mono font-light ${isSelected ? 'glow-text-cyan' : 'text-stone-950'}`}>
                      0{split.daysPerWeek}
                    </span>
                    <div className="flex items-center space-x-1">
                      {!isUserCreated && (split.daysPerWeek === 2 || split.daysPerWeek === 3) && (
                        <span className={`inline-flex items-center gap-1 text-[8px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full border ${
                          isSelected ? 'bg-stone-800 text-stone-300 border-stone-700' : 'bg-stone-100 text-stone-700 border-stone-200'
                        }`}>
                          <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ backgroundColor: 'var(--hud-lime)' }} />
                          Recommended
                        </span>
                      )}
                      {isUserCreated && (
            ...
            <div>
              <div className="flex items-center space-x-3 mb-2">
                <span className="px-3 py-0.5 bg-stone-900 text-white text-[10px] font-bold uppercase tracking-widest rounded-full">
                  <span className="font-mono">{activeSplit.daysPerWeek}</span> Days / Week
                </span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="p-4 bg-stone-50 border border-stone-200/80 rounded-2xl space-y-1">
              ...
              <p className="text-xs text-stone-600 font-normal truncate">
                <span className="font-mono glow-text-cyan">{activeSplit.days.reduce((acc, d) => acc + d.exercises.length * 3, 0)}</span> total working sets across split
              </p>
            </div>
                ...
                <div key={day.dayNumber} className="bg-stone-50 p-5 rounded-2xl border border-stone-200/80 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-bold text-stone-900 uppercase tracking-widest">
                      Session <span className="font-mono">0{day.dayNumber}</span>
                    </span>
      ...
      {splitToDelete && (
        <ConfirmModal
          title="Delete Custom Split?"
          message={<>Delete <strong className="text-stone-900 font-semibold">{splitToDelete.name}</strong>? This custom frequency split will be permanently removed.</>}
          confirmLabel="Yes, Delete"
          onCancel={() => setSplitToDelete(null)}
          onConfirm={() => {
            if (onDeleteCustomSplit) {
              onDeleteCustomSplit(splitToDelete.id);
            }
