...
export const AddWarmupModal: React.FC<AddWarmupModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-stone-950/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-6 overflow-y-auto">
      <div className="bg-white border border-stone-200 rounded-3xl overflow-hidden max-w-3xl sm:max-w-4xl w-full relative shadow-2xl text-stone-900 my-auto max-h-[92vh] h-[88vh] flex flex-col hud-corners">
        {/* Header */}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1">
                ...
                <HudSelect
                  ...
                  options={[
                    { value: 'Pre-Workout Activation', label: 'Pre-Workout Activation' },
                    { value: 'Hip Mobility & Flow', label: 'Hip Mobility & Flow' },
                    { value: 'Post-Workout Cool Down', label: 'Post-Workout Cool Down' },
                    { value: 'Deactivation & Release', label: 'Deactivation & Release' }
                  ]}
                />
              </div>
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="w-full bg-stone-50 border border-stone-300 rounded-xl px-3.5 py-2 text-sm font-mono text-stone-900 focus:outline-none focus:border-stone-900"
                />
              </div>
            </div>
      ...
      {stepToRemove !== null && (
        <ConfirmModal
          title="Remove Step?"
          message={<>Remove Step <strong className="text-stone-900 font-semibold">{stepToRemove + 1}</strong>{steps[stepToRemove]?.name ? <> ("{steps[stepToRemove].name}")</> : null}?</>}
