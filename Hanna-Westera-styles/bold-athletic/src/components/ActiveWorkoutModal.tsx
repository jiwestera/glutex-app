...
export const ActiveWorkoutModal: React.FC<ActiveWorkoutModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-stone-50 text-stone-900 flex flex-col overflow-hidden">
      ...
      {restTimerSeconds > 0 && (
        <div className="bg-[#d4ff3f] text-black px-6 py-2.5 flex items-center justify-between shadow-md text-xs font-bold border-b-2 border-black animate-fadeIn">
          <div className="flex items-center space-x-2">
            <Clock className="w-4 h-4 text-black animate-spin" />
            <span className="tracking-wide uppercase">Rest Period: <strong className="font-mono text-black">{formatTime(restTimerSeconds)}</strong> remaining</span>
          </div>

          <div className="flex items-center space-x-2">
            <button
              onClick={() => setRestTimerSeconds((prev) => prev + 30)}
              className="bg-black hover:bg-white text-[#d4ff3f] hover:text-black border-2 border-black px-2.5 py-1 text-[10px] font-mono"
            >
              +30s
            </button>
            <button
              ...
              className="bg-black hover:bg-white text-[#d4ff3f] hover:text-black border-2 border-black px-2.5 py-1 text-[10px] uppercase font-bold"
            >
              Skip Rest
            </button>
            <button
              onClick={() => setSoundEnabled(!soundEnabled)}
              className="p-1 hover:bg-black hover:text-[#d4ff3f] border-2 border-transparent hover:border-black"
            >
              {soundEnabled ? <Volume2 className="w-4 h-4 text-black" /> : <VolumeX className="w-4 h-4 text-black/50" />}
            </button>
          </div>
        </div>
      )}
      ...
      {showSummary && (
        <div className="fixed inset-0 z-50 bg-stone-950/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white border border-stone-200 rounded-3xl max-w-md w-full p-6 sm:p-8 space-y-6 shadow-2xl text-stone-900">
            <div className="text-center space-y-2">
              <div className="w-12 h-12 rounded-full bg-stone-900 text-white flex items-center justify-center mx-auto shadow-xs">
            ...
            <div className="flex items-center space-x-3 pt-2">
              <button
                ...
                className="w-1/2 py-3 bg-white hover:bg-stone-100 text-black border-2 border-black font-bold text-xs uppercase tracking-wider"
              >
                Return
              </button>
              <button
                onClick={handleFinalSave}
                className="w-1/2 py-3 bg-[#d4ff3f] hover:bg-black text-black hover:text-[#d4ff3f] border-2 border-black font-black text-xs shadow-xs uppercase tracking-wider"
              >
                Save Log
              </button>
            </div>
