...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="bg-white/95 border-b-[3px] border-black text-stone-900 sticky top-0 z-30 shadow-xs transition-colors">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-20">
          ...
          <div className="flex items-center space-x-2.5 sm:space-x-3 cursor-pointer shrink-0" onClick={() => onTabChange('workout')}>
            <div className="w-9 h-9 sm:w-10 sm:h-10 bg-black dark:bg-[#d4ff3f] border-2 border-black dark:border-[#d4ff3f] logo-dumbbell-circle flex items-center justify-center shrink-0">
              <Dumbbell className="w-4 h-4 sm:w-5 sm:h-5 text-white dark:text-black logo-dumbbell-icon transform -rotate-45" />
            </div>
            <div>
              <div className="flex items-center space-x-2 sm:space-x-2.5">
                <h1 className="text-2xl sm:text-3xl md:text-4xl font-black tracking-[0.1em]! text-stone-900 dark:text-white leading-none">
                  GLUTE<span className="text-[#d4ff3f]">X</span>
                </h1>
                {extraDays > 0 ? (
                  <span
                    className="hidden sm:inline-flex text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full bg-amber-100 text-amber-950 border border-amber-300"
                    title={`${daysPerWeek}-Day Base Routine + ${extraDays} Extra Day(s)`}
                  >
                    {daysPerWeek}+{extraDays} Days/Wk
                  </span>
                ) : (
                  <span className="hidden sm:inline-flex text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full bg-stone-200/80 text-stone-700 border border-stone-300/50">
                    {actualDays} Days/Wk
                  </span>
                )}
              </div>
          ...
          <div className="flex items-center space-x-1.5 sm:space-x-2.5 shrink-0">
            {hasActiveWorkout && (
              ...
              >
                <Activity className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-emerald-600 animate-spin" />
            ...
            {/* Streak */}
            <div className="hidden lg:flex items-center space-x-1.5 px-2.5 py-1 bg-white border border-stone-200 text-xs font-bold uppercase tracking-wide text-stone-700 shadow-2xs shrink-0">
              <Flame className="w-3.5 h-3.5 text-[#d4ff3f]" strokeWidth={2.5} />
              <span>{workoutStreak} Streak</span>
            </div>
            ...
            <button
              ...
              className="flex items-center space-x-1.5 px-2.5 py-1.5 sm:px-3 border-2 border-black bg-black text-[#d4ff3f] text-xs font-bold uppercase tracking-wide hover:bg-[#d4ff3f] hover:text-black transition-all cursor-pointer shadow-xs shrink-0"
        ...
        {/* Navigation Bar */}
        <nav className="flex space-x-2 overflow-x-auto no-scrollbar pb-3 pt-2 border-t-2 border-black">
          {[
            ...
            return (
              <button
                ...
                className={`flex items-center space-x-2 px-4 py-1.5 text-xs font-bold uppercase tracking-wide transition-all whitespace-nowrap border-2 cursor-pointer ${
                  isActive
                    ? 'border-black bg-[#d4ff3f] text-black shadow-xs'
                    : 'border-stone-200/80 bg-white/80 text-stone-600 hover:text-stone-950 hover:bg-white hover:border-stone-300'
                }`}
              >
                <Icon className={`w-3.5 h-3.5 ${isActive ? 'text-black' : 'text-stone-500'}`} />
                <span>{tab.label}</span>
                {tab.badge && (
                  <span
                    className={`text-[9px] font-bold px-1.5 py-0.2 uppercase tracking-wider ${
                      isActive ? 'bg-black text-[#d4ff3f]' : 'bg-stone-100 text-stone-800 border border-stone-300'
                    }`}
                  >
                    {tab.badge}
                  </span>
      ...
      {isOptionsOpen && (
        <div className="fixed inset-0 bg-stone-950/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-stone-200 rounded-3xl p-6 sm:p-8 max-w-lg w-full shadow-2xl space-y-6 animate-in fade-in zoom-in-95 duration-150 text-left max-h-[90vh] overflow-y-auto">
            {/* Modal Header */}
      ...
      {isClearModalOpen && (
        <div className="fixed inset-0 bg-stone-950/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-stone-200 rounded-3xl p-6 sm:p-8 max-w-md w-full shadow-2xl space-y-6 animate-in fade-in zoom-in-95 duration-150 text-left">
